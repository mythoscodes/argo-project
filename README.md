...
Argos는 **수강 전 → 수업 중 → 수업 후 → 운영 전체**를 관통하는 AI 분석 플랫폼입니다.

수강 신청 단계에서 AI가 지원자의 역량을 사전 진단하고, 수업 중에는 실시간 이해도를 분석하며, 수업 후에는 개인별 학습 리포트를 생성하고, 운영 차원에서는 이탈 위험을 조기에 감지합니다.
...
> Argos는 **수강 전 → 수업 중 → 수업 후 → 운영** 전체 주기를 AI로 커버합니다.

### 수강 전 — 수준 미스매치 예방

| ID | 기능 | 설명 | 상태 |
|----|------|------|------|
| F0 | 수강생 사전 실력 점검 | 수강 신청 단계에서 AI가 코딩 기초·논리적 사고력·IT 용어 이해도를 진단. 상담사가 문과 출신이라 대화만으로 실력 판별이 어려운 문제를 해결. 결과를 바탕으로 멘토가 적합한 과정을 추천하고, 강사가 반 배정 시 수준 분포를 사전 파악 | ✅ 완료 |

### 수업 중 — 실시간 이해도 파악

| ID | 기능 | 설명 | 상태 |
|----|------|------|------|
| F1 | 수업 세션 관리 | 강사가 세션 생성 → 참여 코드 발급 → 수강생 모바일 접속 | ✅ 완료 |
| F2 | AI 코드 스니펫 퀴즈 자동생성 | 수업 주제 입력 → AI(Gemini 3 Flash)가 출력 예측, 버그 찾기, 빈칸 채우기 등 코딩 특화 퀴즈 생성 (3~15문제, 5개 난이도 프리셋) | ✅ 완료 |
| F3 | 수강생 실시간 퀴즈 응답 | 수강생이 모바일에서 객관식 + 주관식 퀴즈에 실시간 응답 | ✅ 완료 |
| F4 | 실시간 이해도 히트맵 | 응답이 들어올 때마다 개념별 이해도를 녹색(이해)/빨강(미이해) 히트맵으로 실시간 시각화 | ✅ 완료 |
| F5 | AI 강사 코칭 | 이해도 데이터 분석 → "이 개념에서 60%가 약합니다" 같은 실시간 코칭 + 비전공자/경력자 수준별 교수법 제안 | ✅ 완료 |
| F6 | 양방향 피드백 루프 | 보충 설명 후 재퀴즈 → 이해도 변화(델타) 시각화로 교수 개입 효과 확인 | ✅ 완료 |

### 수업 후 — 개인별 학습 경로

| ID | 기능 | 설명 | 상태 |
|----|------|------|------|
| F7 | 수강생 학습 리포트 | 수업 후 개인별 이해도 추이, 약점 개념, 추천 학습 경로 자동 생성 | ✅ 완료 |
| F10 | AI 역량 진단 | 과목별(Spring/React/Python 등) 정기 수준 측정 + 레이더 차트 + 추적 | ✅ 완료 |

### 운영 — 이탈 방지 & 경영 분석

| ID | 기능 | 설명 | 상태 |
|----|------|------|------|
| F8 | 원장 경영 대시보드 | 학원 전체 수업 품질 + 반별 평균 이해도 + 이탈 위험 수강생 자동 감지 + 과목별 수준 분포 | ✅ 완료 |
| F9 | 멘토 이탈 방지 뷰 | AI 이탈 위험 레이더(3-Signal) + 상담 브리핑 자동 생성 + 내부 강의 추천으로 멘토의 선제적 상담 지원 | ✅ 완료 |
...
| 역할 | 시간축 | 핵심 화면 | 화면 수 |
|------|--------|----------|---------|
| 강사 | 지금 이 순간 | 라이브 대시보드 (히트맵 + AI 코칭 + 퀴즈 프리셋) + 수강생 수준 확인 | 4 |
| 수강생 | 수강 전 ~ 수업 후 | 사전 실력 점검 + 퀴즈 응답 + 결과 + 리포트 + 역량 진단 | 7 |
| 원장 | 어제까지 | 경영 대시보드 5탭 (개요/세션/수강생/수준분포/이탈) | 1 (5탭) |
| 멘토 | 내일을 막는다 | 이탈 레이더 + AI 상담 브리핑 + 과정 추천 + 상담 기록 | 2 |
